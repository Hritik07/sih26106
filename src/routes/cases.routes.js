 ...
 * Admin-only — matches who's allowed to see the analyst list in the first
 * place (GET /api/auth/analysts). This is the endpoint the frontend's
 * "Assign analyst" dropdown was 404ing against; it never existed before.
 *
 * Body: { analystId: "<User _id>" } to assign, or { analystId: null } to
 * explicitly unassign (matches the dropdown's "unassigned" option).
 */
router.post('/:id/assign', requireRole('admin'), async (req, res) => {
  try {
    ...
    if (analystId) {
      const analyst = await User.findOne({ _id: analystId, role: 'analyst' });
      if (!analyst) {
        return res.status(400).json({ error: 'INVALID_ANALYST', message: 'No analyst found with that id' });
      }

      caseDoc.assigned_to = analyst._id;
      caseDoc.assigned_by = req.user.id;
      caseDoc.assigned_at = new Date();

      caseDoc.timeline.push({
        stage: 'assigned',
        actor: req.user.id,
        summary: `Assigned to ${analyst.name || analyst.email}`,
        at: caseDoc.assigned_at
      });
    } else {
      caseDoc.assigned_to = null;
      caseDoc.assigned_by = null;
      caseDoc.assigned_at = null;
    ...
    res.status(500).json({ error: 'INTERNAL_ERROR', message: 'Could not update assignment' });
  }
});
