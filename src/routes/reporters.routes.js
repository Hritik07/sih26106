...
/**
 * GET /api/reporters/me/rate-limit
 * Reporter-only. Tells the frontend how many submissions are left in the
 * current rolling 24h window, mirroring the same counting logic
 * middleware/rateLimiter.js uses to actually enforce the cap — this route
 * is read-only and enforces nothing itself, it just reports status so the
 * UI can show "3 of 8 used, resets in 6h" before the reporter even tries
 * to submit.
 */
router.get('/me/rate-limit', verifyJWT, requireRole('reporter'), async (req, res) => {
  const limit = Number(process.env.REPORTER_DAILY_LIMIT || 8);
  ...
  const used = recentCases.length;
  const remaining = Math.max(0, limit - used);
  // resets_at is when the OLDEST submission in the current window rolls
  // out of the 24h lookback — same rolling-window logic as the enforcing
  // middleware, not a fixed midnight reset.
  const resetsAt = used > 0 ? new Date(recentCases[0].createdAt.getTime() + 24 * 60 * 60 * 1000) : null;

  res.json({ data: { limit, used, remaining, resets_at: resetsAt } });
});
