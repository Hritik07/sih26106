...
const { extractPlainBody } = require('../utils/extractPlainBody');
...
async function runAnalysisPipeline(caseDoc, { rawEmail, headers } = {}) {
  // forensics needs the untouched raw MIME source (headers, boundaries,
  // encoding — all of it) to reconstruct relay_path/SPF/DKIM/DMARC.
  // detection does NOT — it was trained on clean subject+body text, and
  // was receiving this same raw string unmodified until now (confirmed
  // root cause of a real confidence-score mismatch: 74% legitimate on
  // clean text vs ~92% phishing through the real pipeline on the same
  // email). Extracted once here, local/no network call, negligible cost.
  const plainBody = await extractPlainBody(rawEmail);

  const [detectionResult, forensicsResult] = await Promise.allSettled([
    detectionService.analyze({ sender: caseDoc.sender, subject: caseDoc.subject, body: plainBody }),
  ...
  // ML/forensics reconciliation — per the detection engineer's recovery
  // plan (Sep 2026): a text-only classifier cannot verify sender
  // authenticity, so a confident phishing/BEC call is NOT auto-trusted if
  // forensics independently shows the sender is who they claim to be.
  // Only requires detection + forensics, not enrichment — runs even if
  // enrichment was skipped/failed. This never auto-clears or overrides an
  // analyst's own flag (checked via !caseDoc.flagged below) — it only ever
  // raises a new one, and only once, on the pipeline's first pass.
  if (caseDoc.detection && caseDoc.forensics && !caseDoc.flagged) {
    const mismatchReason = detectMlForensicsMismatch(caseDoc.detection, caseDoc.forensics);
    if (mismatchReason) {
      caseDoc.flagged = true;
      caseDoc.flag_reason = mismatchReason;
      caseDoc.flagged_at = new Date();
      // flagged_by intentionally left null (its default) — that's how a
      // system-raised flag is distinguished from an analyst's own flag,
      // which always carries a real User ObjectId.
      caseDoc.timeline.push({
        stage: 'flagged',
        actor: 'system',
        summary: mismatchReason,
        at: caseDoc.flagged_at
      });
    }
  }
...
// Confidence floor above which the ML model's call is treated as "strong"
// enough to matter for this check — a low-confidence phishing/BEC guess
// isn't a real disagreement with forensics, it's the model being unsure.
const ML_HIGH_CONFIDENCE_THRESHOLD = 70; // detection.confidence_score is 0-100

/**
 * detectMlForensicsMismatch(detection, forensics)
 *
 * Returns a human-readable reason string if ML and forensics disagree in
 * the specific way the detection engineer's diagnosis called out (a real
 * confirmed failure mode: a genuine Google security-alert email classified
 * as BEC at 100% confidence) — returns null if there's no mismatch, or not
 * enough signal from either side to compare.
 *
 * Deliberately NOT an allowlist (rejected as a security anti-pattern in the
 * same diagnosis, for good reason — sender identity alone isn't proof of
 * authenticity). This never marks anything "safe" or auto-clears a case; it
 * only ever raises a flag for a human to look at when the two signals
 * disagree. A mismatch does not change caseDoc.status or block
 * confirmation — it only sets caseDoc.flagged, which an analyst/admin can
 * see and act on same as any manually-raised flag.
 */
function detectMlForensicsMismatch(detection, forensics) {
  const classification = detection?.classification;
  const confidence = detection?.confidence_score;
  if (!classification || typeof confidence !== 'number') return null;

  const mlSaysThreat = (classification === 'phishing' || classification === 'bec') && confidence >= ML_HIGH_CONFIDENCE_THRESHOLD;
  if (!mlSaysThreat) return null;

  const dkimPass = readDkimStatus(forensics) === 'pass';
  const dmarcPass = readDmarcStatus(forensics) === 'pass';
  const lowSpoofingRisk = forensics?.risk_band === 'low';

  const forensicsSaysAuthentic = dkimPass && dmarcPass && lowSpoofingRisk;
  if (!forensicsSaysAuthentic) return null;

  return (
    `ML classified this email as ${classification} at ${confidence}% confidence, ` +
    `but forensics shows DKIM and DMARC both pass with a low spoofing risk score ` +
    `(${forensics?.spoofing_risk_score ?? 'n/a'}) — sender authenticity appears genuine. ` +
    `Flagged for manual review rather than auto-trusting either signal alone.`
  );
}
